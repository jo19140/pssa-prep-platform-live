...
5. **`PSSA_PASSAGE_MULTIPOINT_EVIDENCE_OVERLAP`**: pairwise keyed-evidence overlap between multipoint items on one passage ≤1 sentence (compare keyed span/choice source sentences after normalization). **CANONICAL-ID RULE (Claude WIP-audit finding, permanent): the gate MUST resolve all three interaction types (EBSR Part B, HOT_TEXT, MULTI_SELECT) through ONE sentence-ID resolver — `sentenceIdForText(text, passage)` over `passageSentences()`. Authored HOT_TEXT `paragraphIndex`/`sentenceIndex` are advisory DISPLAY metadata only and must NOT be used as comparison identity — they diverge from the regex-split ids (verified: map 3:2 vs 3:3, mural 2:3 vs 2:4) and cause silent false-negatives / a false clean.** Required unit test: an HT span whose authored index differs from the computed split, with EBSR/MS citing the same sentence text, all resolve to the same id and flag the overlap. Verified true pre-rekey set with the fix = 17 items (mural/map/cart 3-way; creek/lantern/bell EBSR∩HT; lunch HT∩MS).
...
**PLUS the overlap-gate audit table (Pro requirement):**
- Before gate fix: flagged count (15) + the map-HT/mural-HT miss explanation.
- After gate fix, before re-key: flagged count = 17; per passage — item pair/triple, sentence IDs, keyed text, reason (exact | containment | same-id).
- After re-key: bank-wide overlap failures = 0; changed S2 item list; proof no edits outside EBSR/HT/MS for the S2 portion; the id-space-consistency unit test result.
