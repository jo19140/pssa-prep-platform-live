...
import { PHASE_4_RCONTROLLED, PHASE_4_RCONTROLLED_TARGETS } from "../../lib/content/phase3EntrySeed";
import { detectPatternCandidates, validatePseudowordCandidate } from "../../lib/literacy/pseudowordValidator";

const db = new PrismaClient();

export async function seedPhase4RControlled() {
  const phasePosition = await db.phasePosition.upsert({
    where: {
      phaseNumber_subPosition: {
        phaseNumber: PHASE_4_RCONTROLLED.phaseNumber,
        subPosition: PHASE_4_RCONTROLLED.subPosition,
      },
    },
    create: PHASE_4_RCONTROLLED,
    update: PHASE_4_RCONTROLLED,
  });
  ...
  for (const target of PHASE_4_RCONTROLLED_TARGETS) {
    ...
    await db.dailyTarget.upsert({
      ...
      create: {
        ...
        reviewedAt: new Date(),
        reviewNotes: "Seeded from content-v3 Phase 4 R-Controlled Entry spec.",
      },
      update: {
        ...target,
        phasePositionId: phasePosition.id,
  ...
  return {
    phasePosition,
    dailyTargets: PHASE_4_RCONTROLLED_TARGETS.map((target) => target.code),
  };
}
...
async function main() {
  const result = await seedPhase4RControlled();
...
if (require.main === module) {
  main()
    .then(() => db.$disconnect())
    .catch(async (error) => {
      console.error("Failed to seed Phase 4 R-Controlled Entry content:", error);
      await db.$disconnect();
      process.exit(1);
    });
}
