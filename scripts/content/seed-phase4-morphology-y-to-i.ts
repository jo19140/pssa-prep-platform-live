...
import { PHASE_4_MORPHOLOGY_Y_TO_I, PHASE_4_MORPHOLOGY_Y_TO_I_TARGETS } from "../../lib/content/phase3EntrySeed";
import { detectPatternCandidates, validatePseudowordCandidate } from "../../lib/literacy/pseudowordValidator";

const db = new PrismaClient();

export async function seedPhase4MorphologyYToI() {
  const phasePosition = await db.phasePosition.upsert({
    where: {
      phaseNumber_subPosition: {
        phaseNumber: PHASE_4_MORPHOLOGY_Y_TO_I.phaseNumber,
        subPosition: PHASE_4_MORPHOLOGY_Y_TO_I.subPosition,
      },
    },
    create: PHASE_4_MORPHOLOGY_Y_TO_I,
    update: PHASE_4_MORPHOLOGY_Y_TO_I,
  });

  for (const target of PHASE_4_MORPHOLOGY_Y_TO_I_TARGETS) {
    ...
    await db.dailyTarget.upsert({
      ...
      create: {
        ...
        reviewedAt: new Date(),
        reviewNotes: "Seeded from content-v3 Phase 4 Morphology y to i spec.",
      },
      update: {
        ...target,
        phasePositionId: phasePosition.id,
  ...
  return {
    phasePosition,
    dailyTargets: PHASE_4_MORPHOLOGY_Y_TO_I_TARGETS.map((target) => target.code),
  };
}
...
async function main() {
  const result = await seedPhase4MorphologyYToI();
...
if (require.main === module) {
  main()
    .then(() => db.$disconnect())
    .catch(async (error) => {
      console.error("Failed to seed Phase 4 Morphology y to i content:", error);
      await db.$disconnect();
      process.exit(1);
    });
}
