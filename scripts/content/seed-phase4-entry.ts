...
import { PHASE_4_ENTRY, PHASE_4_ENTRY_TARGETS } from "../../lib/content/phase3EntrySeed";
import { detectPatternCandidates, validatePseudowordCandidate } from "../../lib/literacy/pseudowordValidator";

const db = new PrismaClient();

export async function seedPhase4Entry() {
  const phasePosition = await db.phasePosition.upsert({
    where: {
      phaseNumber_subPosition: {
        phaseNumber: PHASE_4_ENTRY.phaseNumber,
        subPosition: PHASE_4_ENTRY.subPosition,
      },
    },
    create: PHASE_4_ENTRY,
    update: PHASE_4_ENTRY,
  });
  ...
  for (const target of PHASE_4_ENTRY_TARGETS) {
    ...
    await db.dailyTarget.upsert({
      ...
      create: {
        ...
        reviewedAt: new Date(),
        reviewNotes: "Seeded from content-v3 Phase 4 Entry long-vowel teams spec.",
      },
      update: {
        ...target,
        phasePositionId: phasePosition.id,
  ...
  return {
    phasePosition,
    dailyTargets: PHASE_4_ENTRY_TARGETS.map((target) => target.code),
  };
}
...
async function main() {
  const result = await seedPhase4Entry();
...
if (require.main === module) {
  main()
    .then(() => db.$disconnect())
    .catch(async (error) => {
      console.error("Failed to seed Phase 4 Entry content:", error);
      await db.$disconnect();
      process.exit(1);
    });
}
