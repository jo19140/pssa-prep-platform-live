...
import { PHASE_4_MORPHOLOGY, PHASE_4_MORPHOLOGY_TARGETS } from "../../lib/content/phase3EntrySeed";
import { detectPatternCandidates, validatePseudowordCandidate } from "../../lib/literacy/pseudowordValidator";

const db = new PrismaClient();

export async function seedPhase4Morphology() {
  const phasePosition = await db.phasePosition.upsert({
    where: {
      phaseNumber_subPosition: {
        phaseNumber: PHASE_4_MORPHOLOGY.phaseNumber,
        subPosition: PHASE_4_MORPHOLOGY.subPosition,
      },
    },
    create: PHASE_4_MORPHOLOGY,
    update: PHASE_4_MORPHOLOGY,
  });

  for (const target of PHASE_4_MORPHOLOGY_TARGETS) {
    ...
    await db.dailyTarget.upsert({
      ...
      create: {
        ...
        reviewedAt: new Date(),
        reviewNotes: "Seeded from content-v3 Phase 4 Morphology Entry A spec.",
      },
      update: {
        ...target,
        phasePositionId: phasePosition.id,
  ...
  return {
    phasePosition,
    dailyTargets: PHASE_4_MORPHOLOGY_TARGETS.map((target) => target.code),
  };
}
...
async function main() {
  const result = await seedPhase4Morphology();
...
if (require.main === module) {
  main()
    .then(() => db.$disconnect())
    .catch(async (error) => {
      console.error("Failed to seed Phase 4 Morphology content:", error);
      await db.$disconnect();
      process.exit(1);
    });
}
