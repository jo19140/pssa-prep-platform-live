...
import { PHASE_4_TEAMS_CLEANUP, PHASE_4_TEAMS_CLEANUP_TARGETS } from "../../lib/content/phase3EntrySeed";
import { detectPatternCandidates, validatePseudowordCandidate } from "../../lib/literacy/pseudowordValidator";

const db = new PrismaClient();

export async function seedPhase4TeamsCleanup() {
  const phasePosition = await db.phasePosition.upsert({
    where: {
      phaseNumber_subPosition: {
        phaseNumber: PHASE_4_TEAMS_CLEANUP.phaseNumber,
        subPosition: PHASE_4_TEAMS_CLEANUP.subPosition,
      },
    },
    create: PHASE_4_TEAMS_CLEANUP,
    update: PHASE_4_TEAMS_CLEANUP,
  });
  ...
  for (const target of PHASE_4_TEAMS_CLEANUP_TARGETS) {
    ...
    await db.dailyTarget.upsert({
      ...
      create: {
        ...
        reviewedAt: new Date(),
        reviewNotes: "Seeded from content-v3 Phase 4 Teams Cleanup spec.",
      },
      update: {
        ...target,
        phasePositionId: phasePosition.id,
  ...
  return {
    phasePosition,
    dailyTargets: PHASE_4_TEAMS_CLEANUP_TARGETS.map((target) => target.code),
  };
}
...
async function main() {
  const result = await seedPhase4TeamsCleanup();
...
if (require.main === module) {
  main()
    .then(() => db.$disconnect())
    .catch(async (error) => {
      console.error("Failed to seed Phase 4 Teams Cleanup content:", error);
      await db.$disconnect();
      process.exit(1);
    });
}
