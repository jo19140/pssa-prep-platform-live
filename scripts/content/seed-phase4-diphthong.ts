...
import { PHASE_4_DIPHTHONG, PHASE_4_DIPHTHONG_TARGETS } from "../../lib/content/phase3EntrySeed";
import { detectPatternCandidates, validatePseudowordCandidate } from "../../lib/literacy/pseudowordValidator";

const db = new PrismaClient();

export async function seedPhase4Diphthong() {
  const phasePosition = await db.phasePosition.upsert({
    where: {
      phaseNumber_subPosition: {
        phaseNumber: PHASE_4_DIPHTHONG.phaseNumber,
        subPosition: PHASE_4_DIPHTHONG.subPosition,
      },
    },
    create: PHASE_4_DIPHTHONG,
    update: PHASE_4_DIPHTHONG,
  });
  ...
  for (const target of PHASE_4_DIPHTHONG_TARGETS) {
    ...
    await db.dailyTarget.upsert({
      ...
      create: {
        ...
        reviewedAt: new Date(),
        reviewNotes: "Seeded from content-v3 Phase 4 Diphthong Entry spec.",
      },
      update: {
        ...target,
        phasePositionId: phasePosition.id,
  ...
  return {
    phasePosition,
    dailyTargets: PHASE_4_DIPHTHONG_TARGETS.map((target) => target.code),
  };
}
...
async function main() {
  const result = await seedPhase4Diphthong();
...
if (require.main === module) {
  main()
    .then(() => db.$disconnect())
    .catch(async (error) => {
      console.error("Failed to seed Phase 4 Diphthong Entry content:", error);
      await db.$disconnect();
      process.exit(1);
    });
}
