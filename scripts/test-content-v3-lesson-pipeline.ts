...
import { CONTENT_V3_DAILY_TARGETS, PHASE_3_MID_TARGETS, PHASE_4_ENTRY_TARGETS, PHASE_4_MID_TARGETS } from "../lib/content/phase3EntrySeed";
...
function lessonContext(targetCode = "a_e"): LessonGeneratorContext {
  ...
  const isPhase4 = isPhase4Entry || isPhase4Mid;
  const phasePosition = {
    id: isPhase4Mid ? "phase-4-mid" : isPhase4Entry ? "phase-4-entry" : isMid ? "phase-3-mid" : "phase-3-entry",
    phaseNumber: isPhase4 ? 4 : 3,
    label: isPhase4Mid ? "Phase 4 Mid" : isPhase4Entry ? "Phase 4 Entry" : isMid ? "Phase 3 Mid" : "Phase 3 Entry",
  };
  const dailyTarget = {
