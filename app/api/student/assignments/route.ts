...
export async function GET() {
  ...
  const assignments = await db.assignment.findMany({ where: { classRoomId: { in: classIds } }, include: { assessment: true }, orderBy: { createdAt: "desc" } });
  const sessions = await db.testSession.findMany({ where: { userId: (session.user as any).id, assessmentId: { in: assignments.map((a) => a.assessmentId) } }, include: { report: true } });
  return NextResponse.json({ assignments: assignments.map((a) => { const existing = sessions.find((s) => s.assessmentId === a.assessmentId); const statusLabel = existing?.submittedAt ? "Completed" : existing ? "In Progress" : "Not Started"; return { assignmentId: a.id, assessmentId: a.assessmentId, title: a.assessment.title, statusLabel, dueDate: a.dueDate }; }) });
}
