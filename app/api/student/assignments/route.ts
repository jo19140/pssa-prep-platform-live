...
export async function GET() {
  ...
  const latestLearningPath = await db.learningPath.findFirst({
    ...
    include: {
      ...
      lessons: { orderBy: { priority: "asc" }, include: { progress: { where: { userId: (session.user as any).id } }, items: { orderBy: { order: "asc" } } } },
      session: { select: { assessment: { select: { title: true, grade: true } }, submittedAt: true, scorePercent: true } },
    },
    orderBy: { createdAt: "desc" },
  });
  return NextResponse.json({ assignments: assignments.map((a) => { const existing = sessions.find((s) => s.assessmentId === a.assessmentId); const statusLabel = existing?.submittedAt ? "Completed" : existing ? "In Progress" : "Not Started"; return { assignmentId: a.id, assessmentId: a.assessmentId, title: a.assessment.title, assignmentType: a.assignmentType, statusLabel, dueDate: a.dueDate, submittedAt: existing?.submittedAt ?? null, sessionId: existing?.id ?? null }; }), latestLearningPath });
}
