...
import { pssaMinimalMutationResponse, pssaNoStoreHeaders, rejectPssaItem } from "@/lib/content/pssaItemReview";
...
export async function POST(req: Request) {
  ...
  const rate = await consumeRateLimit({
    key: `admin-pssa-review-reject:${auth.user.id}:${getClientIp(req)}`,
    capacity: 60,
  ...
  const result = await rejectPssaItem(db, {
    id: parsed.data.id,
