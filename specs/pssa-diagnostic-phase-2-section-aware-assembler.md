...
## Reading EC repeat policy — approved diagnostic exception
Design decision: Because the fixed Grade 3 stamina diagnostic uses all 20 approved reading MCQ, two correctly labeled reading-MCQ ECs repeat three times in the selected canonical form:
- E03.A-K.1.1.3: boat_02, rabbit_02, rabbit_04
- E03.B-K.1.1.1: owls_01, owls_03, owls_04
These ECs are honestly assigned. Do NOT reclassify items to satisfy a gate.
Therefore:
- GRADE3_DIAGNOSTIC_BLUEPRINT.maxReadingEcRepeats = 3
- GRADE3_BLUEPRINT.maxReadingEcRepeats = 2
This relaxation is diagnostic-only. The foundation blueprint remains unchanged.
Assertions: diagnostic cap is 3; foundation cap is 2; selected diagnostic form has no reading-MCQ EC count above 3; exactly E03.A-K.1.1.3 and E03.B-K.1.1.1 are at count 3.
Backlog: author replacement reading MCQ candidates later if we want to return the diagnostic blueprint to cap=2 without mislabeling items or changing the 45-point form shape.
...
## Hard constraints
Assembler/form logic only. Do not change item content/passage/keys/EBSR evidence/EC/scoring/pssaScoring.ts/schema/migrations/foundation/flat behavior. Do not commit student report PDFs, anchor-analysis exports, student-level data, p2-band-7-8-ae-content files, or tsconfig.tsbuildinfo churn. If the pool cannot satisfy the blueprint deterministically: STOP, report the failing gate/section. Do not relax any gate EXCEPT the approved diagnostic-only change: GRADE3_DIAGNOSTIC_BLUEPRINT.maxReadingEcRepeats = 3. Do not change GRADE3_BLUEPRINT.maxReadingEcRepeats (must remain 2), item ECs, item content, scoring, schema, or foundation behavior.
...
Scope: git diff --name-only main...HEAD — expect only assembler/form logic (including the file containing GRADE3_DIAGNOSTIC_BLUEPRINT, only for maxReadingEcRepeats: 2 → 3) + tests + this spec + (optional) regenerated report. STOP-worthy: GRADE3_BLUEPRINT changes, pssaScoring.ts, schema.prisma, migrations, item fixture content changes, passage text changes, foundation content, p2-band-7-8-ae-content, student data exports, tsconfig.tsbuildinfo.
...
## Stop report
branch; commit SHA; files changed; confirm no content/scoring/schema/foundation changes; confirm no student data committed; GRADE3_DIAGNOSTIC_BLUEPRINT.maxReadingEcRepeats = 3; GRADE3_BLUEPRINT.maxReadingEcRepeats = 2; selected diagnostic EC counts E03.A-K.1.1.3 = 3, E03.B-K.1.1.1 = 3, no EC above 3; no item ECs changed; assembled form summary (section IDs, labels, sectionType, estimatedMinutes, selected units per section, selected item IDs per section, item-type counts per section, point totals per section); whole-form totals (35 items / 45 pts / 4 units / 20 reading / 9 conv / 2 EBSR / 2 TE / 2 SA); selected multipoint (boat_ebsr_01, owls_ebsr_01, syrup_dd_01, boat_mg_01); selected SA (syrup_sa_01, boat_sa_01); omitted (syrup_ebsr_01, rabbit_ebsr_01, owls_06, rabbit_sa_01); owls group proof (1 unit; both members present; both members + all selected owl items in same section; no missing_primary_passage); per-section sum-check; full-form gate results; determinism hash (two runs equal); flat foundation proof (byte-identical, 7/91/12/12/8 hashStable); test results (tsc, build, test:pssa-content, test:pssa-db6, test:pssa-pr-b, relevant PR-C/PR-D); privacy grep result.
