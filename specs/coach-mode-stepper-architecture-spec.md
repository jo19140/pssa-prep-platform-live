...
- **Coordinator unit test (deferred-promise, injected sender):** two immediate captures → **two serialized sender CALLS**; the second receives the first *acknowledged* session id (proves serialization + id reuse, **NOT** database rows; no second send until the prior response supplies/reconfirms the id).
- **Manual two-word grouping smoke (merge gate — proves DB grouping):** two successful POSTs → equal response `voiceSessionId` → **≥2 `LabeledVoiceSegment` rows in that exact session**. The P4A verifier needs only one qualifying segment, so it cannot prove grouping by itself.
...
- **PR-0B** — external **`PseudowordCaptureCoordinator`** (owns `voiceSessionId`, in-flight promise, **background-upload pending** state; **serialized** captures); existing scrolling pseudoword block consumes it via an injectable `captureCoordinator` seam; deferred-promise test. **`hasPending`/`subscribePending` is background-upload state only — NOT the interaction-busy signal (that's the separate `onBusyChange` for recording/VAD/TTS), and must never disable stepper Next/Back.** One-session guarantee holds for *successfully acknowledged* captures; response-loss idempotency (server-issued block key) is out of scope. `wordIndex` source-index preservation for one-word screens is deferred to PR-C. **Authoritative detail: `specs/coach-mode-pr0b-pseudoword-capture-coordinator-spec.md`.**
