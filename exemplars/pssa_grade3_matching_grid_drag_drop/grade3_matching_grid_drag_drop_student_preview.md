# Grade 3 PSSA Matching Grid + Drag Drop Student Preview

Review status: PENDING. Item status: candidate. Internal scoring details are not shown.
...
Complete the table to show how details in the creek passage support the main idea.

Choose one column for each row.

| Row | Observed detail | Explanation clue | Later result |
|---|---|---|---|
| Maya drew a map of three creek spots. | [ ] | [ ] | [ ] |
| Two warm days followed a heavy rain. | [ ] | [ ] | [ ] |
| The glow had faded to a faint stripe. | [ ] | [ ] | [ ] |

### pssa_dd_g3_creek_01

Put the class's creek investigation events in the order they happened.

Drag each event into the correct order.

Tokens:
1. The class wrote a creek notice for families.
2. Maya noticed a pale green shine along Pine Creek.
3. Students compared their notes with the weather chart.
...
Complete the table to show how map details help visitors compare past and present.

Choose one column for each row.

| Row | Old map detail | Present-day change | Still in place |
|---|---|---|---|
| Some streets had new names. | [ ] | [ ] | [ ] |
| The map showed trolley stops. | [ ] | [ ] | [ ] |
| The river helped visitors compare the maps. | [ ] | [ ] | [ ] |

### pssa_dd_g3_map_01

Sort the details by how they help visitors compare the old town and the present town.

Drag each detail into the best bucket. Leave the extra detail out.

Tokens:
1. Trolley tracks were gone.
2. The river helped compare the maps.
3. A date in the corner said 1928.
4. Some streets had new names.

Targets:
- Changed from past to present (2)
- Helped compare both maps (1)
...
Complete the table to show the lunch-line problem, change, and result.

Choose one column for each row.

| Row | Problem noticed | Change tried | Result checked |
|---|---|---|---|
| Most bowls of soup were still warm. | [ ] | [ ] | [ ] |
| Students had to step backward for a napkin. | [ ] | [ ] | [ ] |
| Milk moved to the first table. | [ ] | [ ] | [ ] |

### pssa_dd_g3_lunch_01

Put the lunch-line work in the order the class followed.

Drag each event into the correct order.

Tokens:
1. Milk moved to the first table.
2. Students counted how many times the line paused.
3. The class noticed two slow spots.
...
Complete the table to show how the mural story develops its message.

Choose one column for each row.

| Row | Setting detail | Response to mistake | Lesson shown |
|---|---|---|---|
| The narrator painted tiny waves around each fish tail. | [ ] | [ ] | [ ] |
| The wall no longer looked empty. | [ ] | [ ] | [ ] |
| Chalk lines crossed the brick wall like a giant puzzle. | [ ] | [ ] | [ ] |

### pssa_dd_g3_mural_01

Sort the details by how they show the mural story's message.

Drag each detail into the best bucket. Leave the extra detail out.

Tokens:
1. Grandpa said to turn mistakes into ripples.
2. The fish seemed to move.
3. The narrator had a purple soccer bruise.
4. The wall no longer looked empty.

Targets:
- Mistake changed into art (1)
- Good result of the change (2)
...
Complete the table to show the cart problem, fix, and lesson.

Choose one column for each row.

| Row | Problem | Repair step | Lesson |
|---|---|---|---|
| Check wheels before loading paper. | [ ] | [ ] | [ ] |
| A teacher snipped the yarn with small scissors. | [ ] | [ ] | [ ] |
| The front wheel wobbled left. | [ ] | [ ] | [ ] |

### pssa_dd_g3_cart_01

Put the cart repair steps in the order the passage explains.

Drag each step into the correct order.

Tokens:
1. Add one drop of oil.
2. Wipe the axle with a damp cloth.
3. Snip the yarn with small scissors.
