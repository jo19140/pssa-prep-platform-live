# Grade 3 PSSA EBSR Student Preview

Review status: PENDING. Item status: candidate. Answers, rationales, and rubrics are not shown.
...
What is the main idea of the creek passage?

A. The class uses creek clues to ask why the glow is strongest in one place.
B. Maya wants families to decide that Pine Creek is unsafe after the heavy rain.
C. The teacher asks students to collect creek water in jars for a classroom test.
D. Neighbors learn that robins and beetles caused the pale green creek glow.

**Part Two**

Which two details best support the answer in Part One?
...
Which two pieces of evidence best support the answer in Part One?
...
2. The class noticed two slow spots.
3. At lunch, the line still made noise, but it moved more smoothly.
...
How does following the repair steps help the class solve the cart problem?

A. The steps help the class find the small cause before loading the cart.
B. The steps show that a hammer is the best tool for repairing a stuck wheel.
C. The steps prove that old carts should be replaced whenever they squeak loudly.
D. The steps help students paint the cart green before moving it to the sink.
