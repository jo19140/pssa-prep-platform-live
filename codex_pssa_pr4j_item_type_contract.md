...
**4. MCQ-style subpart safeguards.** Any single-answer selected-response component — including EBSR Part A — inherits the Grade 3 MCQ gates: exactly one defensible correct answer; plausible passage-specific distractors; distractor roles; no generic distractors; no copied-choice shortcut; no duplicate/near-duplicate stem; no giveaway absolutes; no length shortcut.

**4a. Production position / surface-shortcut safeguards — ENFORCED, not merely reported (the #4k lesson).** Any displayed selected-response or selectable-option surface must **enforce** shortcut-distribution safeguards, not merely report them. This includes: **MCQ** answer positions; **EBSR Part A** answer positions; **EBSR Part B** evidence-option positions; **MULTI_SELECT** choice positions; **INLINE_DROPDOWN** option ordering when repeated generated items could create a shortcut. Batch-level FAIL conditions for choice-list surfaces: all items share one correct position; any one position is correct in more than `ceil(n/4)` items (document any small-n exception); the correct set is always the first k options. For **HOT_TEXT**, **MATCHING_GRID**, and **DRAG_DROP**, enforce the surface-equivalent shortcut gate: no always-first / always-top / always-earliest selected span; no repeated same row/column/token-position shortcut; no unmarked option/span/cell/token that is equally valid; **no correct-answer pattern that can be guessed from screen position without reading.** Batch-level gates are required wherever the shortcut appears only across a tranche, not inside a single item. (#4k shipped 5/5 EBSR with Part A correct = A on all five and Part B correct = first-two on all five, because only per-item single-defensibility was inherited, not a batch position gate. That cannot recur.)

**5. Multi-response / TEI safeguards.** For EBSR Part B, multiple-select, hot-text, matching/grid, and drag/drop: instruction matches correct-response count; no unmarked option/span/cell/token is equally correct; no correct option is unsupported; partial-credit rules explicit; scoring never awards full credit for unsupported evidence; every selected evidence span is verbatim and specific enough to support the answer; every distractor span/token/cell is plausible but wrong. (Position safeguard for these surfaces is covered by 4a's surface-equivalent shortcut gate — e.g. EBSR Part B correct-pair positions must vary across a tranche.)
