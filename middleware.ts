...
export default async function middleware(req: Request & { nextUrl: URL; url: string }) {
  ...
  if (isProtectedPath(pathname) && !token) {
  ...
  } else if (pathname === "/parent" && role === "PARENT") {
    response = NextResponse.redirect(new URL("/parent/literacy", req.url));
    ...
    else if (role === "PARENT") response = NextResponse.redirect(new URL("/parent/literacy", req.url));
